...
ALTER TABLE execution ADD COLUMN update_time timestamp NULL;
...
-- cw: chanage fields from 255 to different lengths to avoid unique key issue.
...
ALTER TABLE execution MODIFY COLUMN vendor_type varchar(64);
ALTER TABLE schedule MODIFY COLUMN vendor_type varchar(64);
ALTER TABLE schedule ADD COLUMN extra_attrs JSON;
ALTER TABLE task MODIFY COLUMN vendor_type varchar(64);
...
-- cw: this should not be all text fields. change some to varchar
-- cw: TODO: adjust it when used.
...
CREATE TABLE IF NOT EXISTS `vulnerability_record` (
    ...
    `fixed_version` varchar(32),
    `urls` text,
    `cvss_score_v3` double precision,
    `cvss_score_v2` double precision,
    `cvss_vector_v3` varchar(255),
    `cvss_vector_v2` varchar(255),
    `description` text,
    `cwe_ids` text,
...
CREATE TABLE IF NOT EXISTS `report_vulnerability_record` (
    ...
    `vuln_record_id` bigint(20) unsigned NOT NULL,
